...
void DFA::graph(string output_file)
{
    ...
    {
        add_edge(add_vertex(Vertex{edges_no_trans[i].first}, g), add_vertex(Vertex{edges_no_trans[i].second}, g), Edge{transitions[i]}, g);
        // std::cout << edges_no_trans[i].first << ":::" << edges_no_trans[i].second << "==" << transitions[i] << endl;
    }
    ...
    //Write graph to file
    {
        ...
        // dp.property("label", get(&Vertex::id, g));
